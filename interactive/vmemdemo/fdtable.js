...
joint.shapes.html.Element = joint.shapes.basic.Rect.extend({
    defaults: joint.util.deepSupplement({
        type: 'html.Element',
        attrs: {
            rect: { stroke: 'none', 'fill-opacity': 1, 'fill' : '#ffffff' }
        }
...
joint.shapes.html.ElementView = joint.dia.ElementView.extend({
	...
	'<div class="pmenu">',
	'<span class="phead">Process</span>',
	'<button>fork</button>',
	'<button>pipe</button>',
	'<button>exit</button><br>',
	'<button id="bopen">open</button>(<input type="text" id="fname" size="5"/>)<br>',
	'<button id="bclose">close</button>(' + selectfile("c1")  + ')<br>',
	'<button id="bdup2">dup2</button>(' + selectfile("d1") + ',' + selectfile("d2") + ')<br>',
	'<button id="bread">read</button>(' + selectfile("r1") + ',' + selectfile("r2",[1,2,3,4,5,10,15,20]) + ')<br>',
	'</div>',
        '<label></label>',
	'<table></table><br/>',
        '</div>'
    ...
    this.$box.find('#c1').on('change', _.bind(function(evt) {
		   this.$box.find('#bclose').trigger('click');
    }, this));
    this.$box.find('#d2').on('change', _.bind(function(evt) {
		   this.$box.find('#bdup2').trigger('click');
    }, this));
    this.$box.find('#22').on('change', _.bind(function(evt) {
		   this.$box.find('#bread').trigger('click');
    }, this));
	/*
...
function connections() {
	...
	this.connect = function(s1, f, s2) {
		...
		var tcell = $('#str_' + s1.id + '_' + f)
		var t = tcell.offset();
		var ptr1 = new joint.shapes.standard.Circle({ position: { x: t.left + tcell.width()-25, y: t.top-2 },
		...
		var pl = new joint.dia.Link({
		    source: { id: ptr1.id },
		    target: { id: grel.id, anchor: { name: 'topLeft', args: { dx:-10 } }},
		    router: { name: 'manhattan',
					  args: {
					  padding: 25,
					  perpendicular: true,
					  startDirections: ['right'],
					  endDirections: [(grel == s2.outP) ? 'right' : 'left']}},
			/*router: { name: 'oneSide',
					  args: { side: 'left'}},*/
	...
	this.update = function () {
		for (var i in this.edges) {
			paper.findViewByModel(this.edges[i].lel).update();
		}
	}
}
...
function file_str(id,mod) {
	...
	fileslots[sl] = true;
	var t =  struct(id, "file", {x: 300, y: 10+sl*130}, 110, 120, { mode: mod, nrefs: 1, offset: 0, inode: ""});
	t.slot = sl;
	t.type = "file";
	return t;
}
...
function inode_str(id, name) {
	...
	inodeslots[sl] = true;
	var t =  struct(id, "inode",{x: 500, y: 10+sl*150} , 150, 60, { path: name });
	t.slot = sl;
	t.type = "inode";
	return t;
}
...
function pipe_str(id) {
	...
	obj.outP = new joint.shapes.basic.Circle({ position: { x: x+115,y:y+10 }, size: { width: 10, height: 10}});
...
function fdtable(id) {
	...
	var obj =  struct(id, "File Descriptors",pos, 180, 360, fields,true);
	obj.slot = sl;
	obj.nchildren = 0;
